...
// TestMissingRequiredMembersConditional covers the members the model requires
// only under a sibling mode, which Smithy required flags do not express.
func TestMissingRequiredMembersConditional(t *testing.T) {
	cases := []struct {
		name  string
		...
		{
			name: "lustre read cache USER_PROVISIONED without a size",
			input: &svcsdk.UpdateFileSystemInput{
				LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
					DataReadCacheConfiguration: &svcsdktypes.LustreReadCacheConfiguration{
						SizingMode: svcsdktypes.LustreReadCacheSizingModeUserProvisioned,
					},
				},
			},
			want: "LustreConfiguration.DataReadCacheConfiguration.SizeGiB",
		},
		{
			name: "openzfs read cache USER_PROVISIONED without a size",
			input: &svcsdk.UpdateFileSystemInput{
				OpenZFSConfiguration: &svcsdktypes.UpdateFileSystemOpenZFSConfiguration{
					ReadCacheConfiguration: &svcsdktypes.OpenZFSReadCacheConfiguration{
						SizingMode: svcsdktypes.OpenZFSReadCacheSizingModeUserProvisioned,
					},
				},
			},
			want: "OpenZFSConfiguration.ReadCacheConfiguration.SizeGiB",
		},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := missingRequiredMembers(tc.input)
			found := false
			for _, g := range got {
				if strings.HasPrefix(g, tc.want) {
					found = true
				}
			}
			if !found {
				t.Errorf("%s missing from %v", tc.want, got)
			}
		})
	}

	// Away from USER_PROVISIONED the size is meant to be absent.
	ok := []*svcsdk.UpdateFileSystemInput{
		{
			LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
				DataReadCacheConfiguration: &svcsdktypes.LustreReadCacheConfiguration{
					SizingMode: svcsdktypes.LustreReadCacheSizingModeNoCache,
				},
			},
		},
		{
			OpenZFSConfiguration: &svcsdktypes.UpdateFileSystemOpenZFSConfiguration{
				ReadCacheConfiguration: &svcsdktypes.OpenZFSReadCacheConfiguration{
					SizingMode: svcsdktypes.OpenZFSReadCacheSizingModeProportionalToThroughputCapacity,
				},
			},
		},
		// A size supplied alongside USER_PROVISIONED is complete.
		{
			LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
				DataReadCacheConfiguration: &svcsdktypes.LustreReadCacheConfiguration{
					SizingMode: svcsdktypes.LustreReadCacheSizingModeUserProvisioned,
					SizeGiB:    aws.Int32(256),
				},
			},
		},
	}
	for i, input := range ok {
		if got := missingRequiredMembers(input); len(got) != 0 {
			t.Errorf("case %d: got %v, want none", i, got)
		}
	}

	// Iops is "USER_PROVISIONED mode only" but not required, so demanding it
	// would reject updates FSx accepts.
	iopsOmitted := &svcsdk.UpdateFileSystemInput{
		LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
			MetadataConfiguration: &svcsdktypes.UpdateFileSystemLustreMetadataConfiguration{
				Mode: svcsdktypes.MetadataConfigurationModeUserProvisioned,
			},
		},
	}
	if got := missingRequiredMembers(iopsOmitted); len(got) != 0 {
		t.Errorf("metadata Iops must not be treated as required: %v", got)
	}
}

// TestRequestCarriesPath pins the check that decides whether an update
// converges: UpdateFileSystem overwrites only the members a request sets, so a
// change the request does not carry is discarded and the delta recurs forever.
func TestRequestCarriesPath(t *testing.T) {
	input := &svcsdk.UpdateFileSystemInput{
		FileSystemId: aws.String("fs-0123456789abcdef0"),
		// 0 is a meaningful value, not an absence: it disables automatic backups.
		LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
			AutomaticBackupRetentionDays: aws.Int32(0),
			DataCompressionType:          svcsdktypes.DataCompressionTypeLz4,
			LogConfiguration: &svcsdktypes.LustreLogCreateConfiguration{
		...
		WindowsConfiguration: &svcsdktypes.UpdateFileSystemWindowsConfiguration{
			SelfManagedActiveDirectoryConfiguration: &svcsdktypes.SelfManagedActiveDirectoryConfigurationUpdates{
				UserName: aws.String("svc"),
			},
		},
		OntapConfiguration: &svcsdktypes.UpdateFileSystemOntapConfiguration{
			RemoveRouteTableIds: []string{"rtb-1", "rtb-2"},
			DiskIopsConfiguration: &svcsdktypes.DiskIopsConfiguration{
				Iops: aws.Int64(3000),
			},
		},
		OpenZFSConfiguration: &svcsdktypes.UpdateFileSystemOpenZFSConfiguration{
			ThroughputCapacity: aws.Int32(160),
		},
	}
	...
	cases := []struct {
		...
		// A non-nil pointer counts even when it addresses the zero value.
		{"Spec.LustreConfiguration.AutomaticBackupRetentionDays", true, true},
		// Enums are not pointers, so presence reads as non-empty.
		{"Spec.LustreConfiguration.DataCompressionType", true, true},
		{"Spec.LustreConfiguration.LogConfiguration.Destination", true, true},
		// The removed leaf inside a struct the builder still produced.
		{"Spec.LustreConfiguration.LogConfiguration.Level", false, true},
		// The reviewer's case: the AD block survives, the OU does not.
		{"Spec.WindowsConfiguration.SelfManagedActiveDirectoryConfiguration.UserName", true, true},
		{"Spec.WindowsConfiguration.SelfManagedActiveDirectoryConfiguration." +
			"OrganizationalUnitDistinguishedName", false, true},
		// Spec-to-SDK member renames.
		{"Spec.OntapConfiguration.DiskIOPSConfiguration.IOPS", true, true},
		{"Spec.WindowsConfiguration.SelfManagedActiveDirectoryConfiguration.DNSIPs", false, true},
		// RouteTableIDs has no single counterpart; a removal is carried by
		// RemoveRouteTableIds.
		{"Spec.OntapConfiguration.RouteTableIDs", true, true},
		{"Spec.OpenZFSConfiguration.RouteTableIDs", false, true},
		// An absent enclosing struct cannot carry anything.
		{"Spec.WindowsConfiguration.FsrmConfiguration.FsrmServiceEnabled", false, true},
		// Paths with no counterpart at all are left to the payload probe.
		{"Spec.WindowsConfiguration.Aliases", false, false},
		{"Spec.SubnetIDs", false, false},
		{"Spec.LustreConfiguration.LogConfiguration.Level.Nested", false, false},
		{"Spec", false, false},
		{"LustreConfiguration.DataCompressionType", false, false},
	}
	for _, tc := range cases {
		t.Run(tc.path, func(t *testing.T) {
			carried, known := requestCarriesPath(input, tc.path)
			if carried != tc.carried || known != tc.known {
				t.Errorf("got (carried=%v, known=%v), want (carried=%v, known=%v)",
					carried, known, tc.carried, tc.known)
			}
		})
	}

	if _, known := requestCarriesPath(nil, "Spec.StorageCapacity"); known {
		t.Error("a nil request must report unknown")
	}

	// An explicitly empty list is a value, not an absence: the smithy serializers
	// gate on `v == nil`, so a non-nil empty slice is written as an explicit [].
	empty := &svcsdk.UpdateFileSystemInput{
		LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
			RootSquashConfiguration: &svcsdktypes.LustreRootSquashConfiguration{
				NoSquashNids: []string{},
			},
		},
	}
	const nidsPath = "Spec.LustreConfiguration.RootSquashConfiguration.NoSquashNids"
	if carried, known := requestCarriesPath(empty, nidsPath); !known || !carried {
		t.Errorf("an explicit empty list must count as carried: got (carried=%v, known=%v)",
			carried, known)
	}
	// A nil list is the absence, and must not.
	omitted := &svcsdk.UpdateFileSystemInput{
		LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
			RootSquashConfiguration: &svcsdktypes.LustreRootSquashConfiguration{
				RootSquash: aws.String("65534:65534"),
			},
		},
	}
	if carried, known := requestCarriesPath(omitted, nidsPath); !known || carried {
		t.Errorf("a nil list must not count as carried: got (carried=%v, known=%v)",
			carried, known)
	}
}

// TestPreflightAllowsExplicitEmptyList: the Lustre guide documents
// NoSquashNids=[] as the way to remove every root-squash client exception, so
// emptying a non-empty list must reach FSx rather than being rejected.
func TestPreflightAllowsExplicitEmptyList(t *testing.T) {
	rm := &resourceManager{}
	ctx := context.Background()
	...
	applied := svcapitypes.FileSystemSpec{
		FileSystemType: aws.String("LUSTRE"),
		LustreConfiguration: &svcapitypes.CreateFileSystemLustreConfiguration{
			RootSquashConfiguration: &svcapitypes.LustreRootSquashConfiguration{
				RootSquash:   aws.String("65534:65534"),
				NoSquashNids: []*string{aws.String("10.0.1.6@tcp"), aws.String("10.0.1.7@tcp")},
			},
		},
	}
	latest := baselineFrom(applied)
	desired := newResource(svcapitypes.FileSystemSpec{
		...
		LustreConfiguration: &svcapitypes.CreateFileSystemLustreConfiguration{
			RootSquashConfiguration: &svcapitypes.LustreRootSquashConfiguration{
				RootSquash: aws.String("65534:65534"),
				// Explicitly empty, not omitted -- `noSquashNids: []` in the CR.
				NoSquashNids: []*string{},
			},
		},
	})
	...
	const path = "Spec.LustreConfiguration.RootSquashConfiguration.NoSquashNids"
	input, err := rm.newUpdateRequestPayload(ctx, desired, latest, deltaAt(path))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	// The premise: the request carries an explicit empty list, which the SDK
	// serializes as [] because it is non-nil.
	if input == nil || input.LustreConfiguration == nil ||
		input.LustreConfiguration.RootSquashConfiguration == nil {
		t.Fatalf("expected a root squash payload, got %+v", input)
	}
	nids := input.LustreConfiguration.RootSquashConfiguration.NoSquashNids
	if nids == nil {
		t.Fatal("NoSquashNids must be non-nil so the SDK writes an explicit []")
	}
	if len(nids) != 0 {
		t.Fatalf("NoSquashNids should be empty, got %v", nids)
	}

	if err := rm.rejectUnexpressibleChanges(ctx, desired, latest, deltaAt(path)); err != nil {
		t.Errorf("emptying NoSquashNids is a documented operation and must not be rejected: %v", err)
	}
}

// TestRejectListsBelowModelMinimum: DnsIps is the one list reachable from
// UpdateFileSystemRequest whose model minimum is not zero, so its empty form is
// an invalid request rather than an update.
func TestRejectListsBelowModelMinimum(t *testing.T) {
	rm := &resourceManager{}
	ctx := context.Background()
	const path = "Spec.WindowsConfiguration.SelfManagedActiveDirectoryConfiguration.DNSIPs"

	t.Run("violatedListMinimums", func(t *testing.T) {
		below := &svcsdk.UpdateFileSystemInput{
			WindowsConfiguration: &svcsdktypes.UpdateFileSystemWindowsConfiguration{
				SelfManagedActiveDirectoryConfiguration: &svcsdktypes.SelfManagedActiveDirectoryConfigurationUpdates{
					DnsIps: []string{},
				},
		...
		if got := violatedListMinimums(below, []string{path}); len(got) != 1 {
			t.Errorf("an empty DnsIps must be reported: got %v", got)
		}
		at := &svcsdk.UpdateFileSystemInput{
			WindowsConfiguration: &svcsdktypes.UpdateFileSystemWindowsConfiguration{
				SelfManagedActiveDirectoryConfiguration: &svcsdktypes.SelfManagedActiveDirectoryConfigurationUpdates{
		...
		if got := violatedListMinimums(at, []string{path}); len(got) != 0 {
			t.Errorf("a list at the minimum must pass: got %v", got)
		}
		// An omitted list is not a minimum violation; it is handled as a removal.
		omitted := &svcsdk.UpdateFileSystemInput{
			WindowsConfiguration: &svcsdktypes.UpdateFileSystemWindowsConfiguration{
				SelfManagedActiveDirectoryConfiguration: &svcsdktypes.SelfManagedActiveDirectoryConfigurationUpdates{},
			},
		}
		if got := violatedListMinimums(omitted, []string{path}); len(got) != 0 {
			t.Errorf("an omitted list must not be reported: got %v", got)
		}
		// Unconstrained lists are never reported, however empty.
		nids := &svcsdk.UpdateFileSystemInput{
			LustreConfiguration: &svcsdktypes.UpdateFileSystemLustreConfiguration{
				RootSquashConfiguration: &svcsdktypes.LustreRootSquashConfiguration{
					NoSquashNids: []string{},
				},
		...
		if got := violatedListMinimums(
			nids, []string{"Spec.LustreConfiguration.RootSquashConfiguration.NoSquashNids"},
		); len(got) != 0 {
			t.Errorf("NoSquashNids has no minimum: got %v", got)
		}
	})

	t.Run("preflight rejects an emptied DnsIps", func(t *testing.T) {
		applied := svcapitypes.FileSystemSpec{
			FileSystemType: aws.String("WINDOWS"),
			WindowsConfiguration: &svcapitypes.CreateFileSystemWindowsConfiguration{
				SelfManagedActiveDirectoryConfiguration: &svcapitypes.SelfManagedActiveDirectoryConfiguration{
					DNSIPs: []*string{aws.String("10.0.0.1")},
				},
			},
		}
		latest := baselineFrom(applied)
		...
		err := rm.rejectUnexpressibleChanges(ctx, desired, latest, deltaAt(path))
		if err == nil {
			t.Fatal("an empty DnsIps is below the model minimum and must be rejected")
		}
		var terminal *ackerr.TerminalError
		if !errors.As(err, &terminal) {
			t.Errorf("error is %T, want *ackerr.TerminalError", err)
		}
		if !strings.Contains(err.Error(), "DNSIPs") {
			t.Errorf("error %q does not name the list", err.Error())
		}
	})
}

// TestRejectUncarriedNestedChanges covers removals the required-member check
// cannot see: the request is built, no required member is missing, and yet the
// change is not carried, so FSx retains the old value and the delta recurs.
func TestRejectUncarriedNestedChanges(t *testing.T) {
	rm := &resourceManager{}
	ctx := context.Background()
	...
	cases := []struct {
		...
		{
			// UpdateFileSystemWindowsConfiguration overwrites only non-null
			// members, so an omitted OU is retained rather than cleared.
			name: "self-managed AD organizational unit",
			applied: svcapitypes.FileSystemSpec{
				FileSystemType: aws.String("WINDOWS"),
				WindowsConfiguration: &svcapitypes.CreateFileSystemWindowsConfiguration{
					SelfManagedActiveDirectoryConfiguration: &svcapitypes.SelfManagedActiveDirectoryConfiguration{
						...
						UserName:                            aws.String("svc"),
						OrganizationalUnitDistinguishedName: aws.String("OU=fsx,DC=corp,DC=example,DC=com"),
					},
				},
			},
			desired: svcapitypes.FileSystemSpec{
			...
			path: "Spec.WindowsConfiguration.SelfManagedActiveDirectoryConfiguration." +
				"OrganizationalUnitDistinguishedName",
		},
		{
			// SizeGiB is required while SizingMode stays USER_PROVISIONED, which
			// no Smithy required flag records.
			name: "lustre read cache size while still USER_PROVISIONED",
			applied: svcapitypes.FileSystemSpec{
				FileSystemType: aws.String("LUSTRE"),
				LustreConfiguration: &svcapitypes.CreateFileSystemLustreConfiguration{
					DataReadCacheConfiguration: &svcapitypes.LustreReadCacheConfiguration{
			...
			path: "Spec.LustreConfiguration.DataReadCacheConfiguration.SizeGiB",
		},
		{
			name: "openzfs read cache size while still USER_PROVISIONED",
			applied: svcapitypes.FileSystemSpec{
				FileSystemType: aws.String("OPENZFS"),
				OpenZFSConfiguration: &svcapitypes.CreateFileSystemOpenZFSConfiguration{
					ReadCacheConfiguration: &svcapitypes.OpenZFSReadCacheConfiguration{
						SizingMode: aws.String("USER_PROVISIONED"),
						SizeGiB:    aws.Int64(128),
					},
				},
			},
			desired: svcapitypes.FileSystemSpec{
			...
			path: "Spec.OpenZFSConfiguration.ReadCacheConfiguration.SizeGiB",
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Guard the premise: a request IS built and, for the AD case, the
			// model reports nothing missing -- so neither earlier check fires and
			// the case genuinely exercises the carried check.
			...
			if input == nil {
				t.Fatal("payload was nil; this case no longer exercises the carried check")
			}
			if carried, known := requestCarriesPath(input, tc.path); !known || carried {
				t.Fatalf("premise broken: got (carried=%v, known=%v)", carried, known)
			}

			if err := rm.rejectUnexpressibleChanges(ctx, desired, latest, single); err == nil {
				t.Fatal("a change the request does not carry must be rejected")
			} else {
				var terminal *ackerr.TerminalError
				if !errors.As(err, &terminal) {
					t.Errorf("error is %T, want *ackerr.TerminalError", err)
			...
			// And rejected before tags are mutated.
			_, err = rm.customUpdateFileSystem(ctx, desired, latest, deltaAt("Spec.Tags", tc.path))
			if err == nil {
				t.Fatal("expected rejection")
			}
			if strings.Contains(err.Error(), "resource ARN") {
				t.Error("tags were synced before the change was rejected")
			}
		})
	}

	// Changing the OU rather than removing it is carried, so it must pass.
	t.Run("changing the organizational unit is allowed", func(t *testing.T) {
		applied := svcapitypes.FileSystemSpec{
			FileSystemType: aws.String("WINDOWS"),
			WindowsConfiguration: &svcapitypes.CreateFileSystemWindowsConfiguration{
				SelfManagedActiveDirectoryConfiguration: &svcapitypes.SelfManagedActiveDirectoryConfiguration{
					OrganizationalUnitDistinguishedName: aws.String("OU=old,DC=corp,DC=example,DC=com"),
				},
			},
		}
		latest := baselineFrom(applied)
		desired := newResource(svcapitypes.FileSystemSpec{
			...
			WindowsConfiguration: &svcapitypes.CreateFileSystemWindowsConfiguration{
				SelfManagedActiveDirectoryConfiguration: &svcapitypes.SelfManagedActiveDirectoryConfiguration{
					OrganizationalUnitDistinguishedName: aws.String("OU=new,DC=corp,DC=example,DC=com"),
				},
			},
		})
		desired.ko.Status = *latest.ko.Status.DeepCopy()
		if err := rm.rejectUnexpressibleChanges(ctx, desired, latest, deltaAt(
			"Spec.WindowsConfiguration.SelfManagedActiveDirectoryConfiguration."+
				"OrganizationalUnitDistinguishedName",
		)); err != nil {
			t.Errorf("unexpected rejection: %v", err)
